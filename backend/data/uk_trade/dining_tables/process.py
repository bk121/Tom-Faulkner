...
nan=float("nan")
wb = load_workbook(filename = '../../master.xlsx', data_only=True)
ws = wb['UK-T-Dining Tables']
...
df = df.drop(df.index[541:564], axis=0)
df.loc[len(df)]=['     ',np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan]
# df.loc[12.5]=[np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan]
# df.loc[27.5]=[np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan]
...
# df2 = pd.read_excel(open('../../../master.xlsx', 'rb'), sheet_name='UK-T-Dining Tables')
